...
SnakeTitle BYTE "       _______  _        _______  _        _______  ",13,10
			 BYTE "      (  ____ \( (    /|(  ___  )| \    /\(  ____ \ ",13,10
			 BYTE "      | (    \/|  \  ( || (   ) ||  \  / /| (    \/ ",13,10
			 BYTE "      | (_____ |   \ | || (__) ||  (_/ / | (_     ",13,10
			 BYTE "      (_____  )| (\ \) ||  ___  ||   _ (  |  __)    ",13,10
			 BYTE "            ) || | \   || (   ) ||  ( \ \ | (       ",13,10
			 BYTE "      /\____) || )  \  || )   ( ||  /  \ \| (____/\ ",13,10
			 BYTE "      \_______)|/    )_)|/     \||_/    \/(_______/ $",13,10
...
PROMPT1 BYTE "Start Game: Press [1]",0
PROMPT2 BYTE "Exit: Press [0]",0
Space_between BYTE " ",0
bonusCounter DWORD 0

.code
main PROC

mov edx,OFFSET SnakeTitle
call writeString
call Crlf
...
EatingCoin PROC
	...
	cmp xPos[esi-2], al	;check if the new tail should be right or left of the old tail
	jl incx
	jg decx
	incx:					;inc if right
	inc xPos[esi]			
...
DrawBonusCoin PROC						; procedure to draw bonus coin
	mov eax,red (red * 16)
	call SetTextColor				;set color to yellow for coin
	mov dl,xBonusPos
	mov dh,yBonusPos
...
CreateRandomBonusCoin PROC				;procedure to create a random coin
	mov eax,49
	call RandomRange	;0-49
	add eax, 35			;35-84
	mov xBonusPos,al
...
checkCoinYPos:
	...
	cmp al, yPos[esi]
jne continueloop			; jump back to continue loop if yPos of snake at esi != yPos of coin
	call CreateRandomBonusCoin		; coin generated on snake, calling function again to create another set of coordinates
CreateRandomBonusCoin ENDP

EatingBonusCoin PROC	; snake is eating bonus coin
	mov BonusCounter,0
	add score, 3
	mov ebx,4
	add bl, score
